...
void Daemon::copy() {
    ...
    for(const auto& entry: std::filesystem::directory_iterator(data.src)) {
        ...
        if((time_now - file_time) >= 180)
            std::filesystem::copy_file(entry.path(), std::filesystem::path(data.dst) / subfolder_old / entry.path().filename(), std::filesystem::copy_options::skip_existing);
        else
            std::filesystem::copy_file(entry.path(), std::filesystem::path(data.dst) / subfolder_new / entry.path().filename(), std::filesystem::copy_options::skip_existing); 
    }
}
