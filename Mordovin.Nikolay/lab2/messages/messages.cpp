...
bool ConnectedQueue::PushFromConnection(Connection *conn) {
    mutex.lock();
    ...
    try {
        conn->Read((void *)&size, sizeof(uint));   // Read messages cnt
        if (size == 0) {
            mutex.unlock();
            return true;
        }
...
bool ConnectedQueue::PopToConnection(Connection *conn) {
    mutex.lock();
